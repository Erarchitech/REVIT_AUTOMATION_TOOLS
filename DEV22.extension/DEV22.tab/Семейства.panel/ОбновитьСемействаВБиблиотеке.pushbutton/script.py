...
for target_value in marks_modified:
    ...
    family_file_path = excel_path[:index+len('01_БИБЛИОТЕКА_ПРОЕКТА')+1] + r"01_СЕМЕЙСТВА/" + Categories_Dict[target_sheet.name][1] + r"/"
...
#     print doc_path
old_doc = False
f_template_path = str(r'C:\\Users\\erarc\\YandexDisk\\NOZ\\00_WIP\\02_PYTHON\\01_DEV\\01_BIM\\01_БИБЛИОТЕКА_ПРОЕКТА\\03_ШАБЛОНЫ\\CP_XX_FN_Box.rfa')
...
for fam_name, fam_data in family_dict.items():
    ...
    doc_path = fam_data[1].replace("\\", "\\\\")
    fam_exist_in_lib_= fam_data[2]
    print fam_name
    print doc
    print library_folder_path
    print '=========================ЗАПИСЬ ДАННЫХ В СЕМЕЙСТВО========================='
    exec("print 'Имя семейства по реестру - '+ '{}'".format(fam_name))
