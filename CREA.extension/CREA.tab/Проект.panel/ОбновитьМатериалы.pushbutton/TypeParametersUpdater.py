                        ...
                        for par_exist in update_type.Parameters:
                            ...
                            if par_new == 'CP_Gen_Image' and par_new == par_exist_name:
                                ...
                                if not File.Exists(image_path):
                                    ...
                                    image_type_collector = FEC(doc).OfClass(ImageType)
                                    import_file_name = param_dict[par_new].split('\\')[-1]
                                    # print image_path
                                    try:
                                        # Ensure valid ImageTypeOptions setup and proper image format
                                        if int(app_version) < 2021:
                                            image_options = ImageTypeOptions(image_path, False)
                                        else:
                                            image_options = ImageTypeOptions(image_path, False, ImageTypeSource.Import)
                                        # print image_options
                                        image_found = False
                                        image_type = None
                                        for image in image_type_collector:
                                            file_name = image.Path.split('\\')[-1]
                                            # print file_name
                                            if file_name == import_file_name:
                                                # print file_name
                                                # print import_file_name
                                                image_found = True
                                                image_type = image.ReloadFrom(image_options)
                                                if image_type:
                                                    print("Изображение найдено и обновлено.")
                                                else:
                                                    image_type = image
                                ...
                                if par_value:
                                    ...
                                    # print par_exist_name
                                    # print param_dict[par_new][1]
