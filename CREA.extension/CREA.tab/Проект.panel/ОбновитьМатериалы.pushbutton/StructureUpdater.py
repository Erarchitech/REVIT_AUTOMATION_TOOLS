...
# Получаем путь к каталогу, где находится текущий файл
current_file_path = os.path.dirname(__file__)
# print current_file_path
# Переходим на несколько уровней вверх к каталогу \REVIT_AUTOMATION_TOOLS
root_path = os.path.abspath(os.path.join(current_file_path, '..', '..', '..', '..'))
# print root_path
# Добавляем путь к \REVIT_AUTOMATION_TOOLS в sys.path
...
def types_updater(doc, type_dict):
    ...
    unit_type = get_unit_type_from_project(doc, spec_type)
    for param_dict in type_dict.values():
        for key, v in param_dict.items():
            if 'CP_Mat_Finish_' in key:
                material_width = v[0]
                material_width_internal = Unit(doc, material_width, unit_type=unit_type).internal
    ...
    with Transaction(doc, 'Обновление типов системных семейств из реестра') as t:
        ...
        for new_type_name, param_dict in type_dict.items():
            ...
            print('\n ================== тип {} - СОЗДАНИЕ ТИПОРАЗМЕРА В REVIT ========================='.format(new_type_name))
            ...
            if not default_type:
                print('!!! ТИП ПО-УМОЛЧАНИЮ НЕ НАЙДЕН !!!')
                sys.exit()
            # print param_dict['Category']
            # print default_type
            ...
            if found_type is None:
                ...
                # print new_type
                print('\n Создан новый типоразмер - {}'.format(new_type_name))
                layers = []
                print('\n               ======== СОЗДАНИЕ СТРУКТУРЫ ========')
                ...
                # print new_type
                print('\n Новый типоразмер добавлен в проект - {}'.format(new_type_name))
                # print('__')
                updated_types.append(new_type)
            else:
                # Обновление существующего типа
                print('\n Найден существующий типоразмер - {}'.format(found_type.Parameter[BuiltInParameter.ALL_MODEL_TYPE_NAME].AsString()))
                ...
                for ind, k in enumerate(sorted_key_list):
                    ...
                    if ind < len(exist_layers):
                        ...
                        if new_material_width != exist_material_width:
                            ...
                            print('Перезаписана толщина слоя в пироге с индексом {}: {}'.format(ind+1, Unit(doc, new_material_width, unit_type=unit_type).display))
                            check+=1
                    else:
                        print(new_material_name)
                        ...
                        check+=1
                if check == 0:
                    print('\n Пирог типа соотвествует реестру')
                else:
                    ...
                    print('\n Существующий типоразмер обновлен - {}'.format(found_type.Parameter[BuiltInParameter.ALL_MODEL_TYPE_NAME].AsString()))
