...
if old_level_name and new_level_name:
    ...
    def get_level_by_name(level_name):
        ...
        # Debugging: log the level names found
        print("Searching for level: {}".format(level_name))
        found_levels = [level.Name for level in collector]
        print("Available levels in the document: {}".format(found_levels))
        
        # Iterate over levels to find the one with the matching name
    ...
    if old_level and new_level:
        ...
        if old_level and new_level:
            ...
            try:
                elevation_difference = calculate_elevation_difference(old_level, new_level)
                
                # Print the elevation difference (in millimeters)
                print("Elevation difference between {} and {}: {} millimeters".format(old_level_name,new_level_name,elevation_difference))
            except Exception as e:
                print("Error calculating elevation difference: {}".format(e))
        else:
            if not old_level:
                print("Level '{}' not found in the document.".format(old_level_name))
            if not new_level:
                print("Level '{}' not found in the document.".format(new_level_name))
