...
if old_level_name and new_level_name:
    ...
    def calculate_elevation_difference(level1, level2):
        if level1 and level2:
            elevation1 = UnitUtils.ConvertFromInternalUnits(level1.Elevation,UnitTypeId.Millimeters)
            elevation2 = UnitUtils.ConvertFromInternalUnits(level2.Elevation,UnitTypeId.Millimeters)
        ...
        with Transaction(doc, 'Изменить уровень элементов') as t:
            ...
            for element in elements:
                ...
                if is_loaded_family(element):
                    ...
                    offset_value = UnitUtils.ConvertFromInternalUnits(offset_param.AsDouble(),UnitTypeId.Millimeters) + calculate_elevation_difference(old_level, new_level)
                    # print(offset_value)
                    if level_param and offset_param:
                        try:
                            level_param.Set(new_level.Id)
                            offset_param.Set(UnitUtils.ConvertToInternalUnits(offset_value,UnitTypeId.Millimeters))
                    ...
                    if element.Category.Id == wall_category.Id:
                        ...
                        offset_value = UnitUtils.ConvertFromInternalUnits(offset_param.AsDouble(),UnitTypeId.Millimeters) + calculate_elevation_difference(doc.GetElement(level_param.AsElementId()), new_level)
                        top_offset_value = UnitUtils.ConvertFromInternalUnits(top_offset_param.AsDouble(),UnitTypeId.Millimeters) + calculate_elevation_difference(doc.GetElement(top_constraint_param.AsElementId()), new_level)
                        if level_param and offset_param:
                            try:
                                offset_param.Set(UnitUtils.ConvertToInternalUnits(offset_value,UnitTypeId.Millimeters))
                                level_param.Set(new_level.Id)
                                top_constraint_param.Set(new_level.Id)
                                top_offset_param.Set(UnitUtils.ConvertToInternalUnits(top_offset_value,UnitTypeId.Millimeters))
                                print('Параметры успешно записаны')
                                counter +=1
                            except:
                                pass
